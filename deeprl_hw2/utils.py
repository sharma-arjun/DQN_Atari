...
def get_hard_target_model_updates(target_name, source_name):
	"""Return list of target model update ops.
	...
	for v in all_vars:
		if v.name.startswith(target_name):
			target_vars.append(v)
		elif v.name.startswith(source_name):
			source_vars.append(v)

	target_vars.sort(key=lambda v: v.name)
	source_vars.sort(key=lambda v: v.name)

	assert(len(target_vars) == len(source_vars))

	hard_copy = [tf.assign(target_vars[i],source_vars[i]) for i in range(len(target_vars))]
