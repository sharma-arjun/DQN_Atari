...
class GreedyEpsilonPolicy(Policy):
	...
	def select_action(self, q_values, **kwargs):
		...
		# Select max Q action with probability 1-eps+eps/num_actions, 
		# Otherwise select other action with probability eps/num_actions

		# bucket = np.zeros(len(q_values))
		# Assuming 9 actions (in Enduro)

		prob = np.ones(self.num_actions)*self.epsilon/self.num_actions
		prob[np.argmax(q_values)] = 1-self.epsilon + self.epsilon/self.num_actions
...
class LinearDecayGreedyEpsilonPolicy(Policy):
	...
	def __init__(self, start_value, end_value, num_steps, num_actions):
	...
	# def select_action(self, **kwargs):
	def select_action(self, q_values, is_training, iteration_number):
		...
		if is_training:
			epsilon = max(self.start_value - iteration_number*(self.start_value-self.end_value)/self.num_steps,self.end_value)
		else:
			epsilon = self.end_value

		prob = np.ones(self.num_actions)*epsilon/self.num_actions
		prob[np.argmax(q_values)] = 1-epsilon + epsilon/self.num_actions
		return np.random.choice(range(self.num_actions),p=prob)
